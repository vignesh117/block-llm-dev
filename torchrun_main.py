...
def evaluate_model(
    ...
    #     for batch in data.batch(batch_size=batch_size):
...
def main(args):
    ...
    if "galore" in args.optimizer.lower():
        logger.info(
            f"Total params with GaLore enabled: {sum(p.numel() for p in galore_params) / 1_000_000:.2f}M"
        )
    ...
    elif args.optimizer.lower() == "blockllm":
        optimizer = AdamBCDConservative(trainable_params, lr=args.lr)
    # implement sgd
    ...
    # Some BCD parameter definitions
    update_freq = math.floor(args.num_training_steps / 4)
    sparsity_level = 0.9
    param_names = []
    param_dist_weights = {}
    total_no_parameters = 0
    if args.optimizer.lower() == "blockllm":
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if (args.optimizer.lower() == "blockllm") and (n_iter % update_freq == 0):
        ...
        if update_step % args.eval_every == 0:
            ...
            total_loss, perplexity_eval, evaluated_on_tokens = evaluate_model(
                model,
                preprocess_batched,
                pad_idx,
                global_rank,
                ...
                wandb.log(
                    {
                        ...
                        "final_perplexity": perplexity.item(),
                    },
                    step=global_step,
                )
    ...
    if global_rank == 0 and not os.path.exists(current_model_directory):
        ...
        os.makedirs(args.save_dir, exist_ok=True)
        model.modules.save_pretrained(current_model_directory)
